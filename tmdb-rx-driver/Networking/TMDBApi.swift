...
protocol TMDBApiMoviesProvider {
    ...
    func searchMoviesAndPersons(forQuery query: String) -> Observable<[Content]?>
}
...
protocol TMDBApiPeopleProvider {
    ...
    func fetchActorDetails(forPersonId id: Int) -> Observable<PersonNew?> 
}
...
final class TMDBApi: TMDBApiProvider {
    ...
    func fetchActorDetails(forPersonId id: Int) -> Observable<PersonNew?> {
        return httpClient.get(url: "https://api.themoviedb.org/3/person/\(id)?api_key=\(Constants.apiKey)&language=en-US)")
            .map { data -> PersonNew? in
                guard let data = data,
                    let response = try? JSONDecoder().decode(PersonNew.self, from: data) else {
                        return nil
                }
                return response
        }
    ...
    func searchMoviesAndPersons(forQuery query: String) -> Observable<[Content]?> {
        return httpClient.get(url: "https://api.themoviedb.org/3/search/multi?api_key=\(Constants.apiKey)&language=en-US&query=\(query)&page=1&include_adult=false")
            .map { data -> [Content]? in
                guard let data = data,
                      let response = try? JSONDecoder().decode(MovieAndPersonResponse.self, from: data) else {
                          return nil
                      }
                return response.results
            }
